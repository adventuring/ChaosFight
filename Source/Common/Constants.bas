          ...
          const BuildDay=BUILD_DAY : rem Build year (4 digits, e.g. 2025, set by Makefile)
          rem Build day in julian day format (1-366, set by Makefile)
          rem Game URL: https://interworldly.com/games/ChaosFight
          const NumArenas=16 : rem URL stored as comment for attribution
          const MaxArenaID=1
          ...
          const CPUCharacter = 254 : rem No character selected ($FF)
          rem CPU player character ($FE)
          
          const MissileLifetimeInfinite = 255 : rem Sentinel and special value constants
          rem Missile lifetime value for infinite (until collision, no
          const MissileHitNotFound = 255 : rem   decrement)
          const MaxByteValue = 255 : rem Sentinel value indicating no hit found in collision checks
          ...
          const PlayerStateAttacking = 4 : rem Bit 0: 1=in recovery/hitstun
          const PlayerStateAnimation = 7 : rem Bit 4: 1=attacking
          ...
          const ActionFallingShifted = 176 : rem ActionFalling (11) << 4 = 176 (0xB0)
          
          const HighBit = $80 : rem High bit constants for input testing
          const PaddleLeftButton = 0 : rem Bit 7: high bit for testing input states
          const PaddleRightButton = 2 : rem INPT0: Left paddle button
          rem INPT2: Right paddle button
          
          const VBlankGroundINPT0123 = $C0 : rem VBLANK constants for controller detection
          rem VBLANK with paddle ground enabled for controller detection
          
          const PlayerEliminatedPlayer0 = 1 : rem Player elimination bit constants (playersEliminated flags)
          const PlayerEliminatedPlayer1 = 2 : rem Bit 0: Player 0 eliminated ($01)
          const PlayerEliminatedPlayer2 = 4 : rem Bit 1: Player 1 eliminated ($02)
          const PlayerEliminatedPlayer3 = 8 : rem Bit 2: Player 2 eliminated ($04)
          rem Bit 3: Player 3 eliminated ($08)
          
          const QuadtariDetected = 7 : rem Controller status bit constants (packed into single byte)
          const Players34Active = 6 : rem Bit 7: Quadtari adapter detected
          rem Bit 6: Players 3 or 4 are active (selected and not
          const LeftPortGenesis = 0 : rem   eliminated) - used for missile multiplexing
          const LeftPortJoy2bPlus = 1 : rem Bit 0: Genesis or Joy2b+ on left port
          const RightPortGenesis = 2 : rem Bit 1: Joy2b+ on left port (subset of LeftPortGenesis)
          const RightPortJoy2bPlus = 3 : rem Bit 2: Genesis or Joy2b+ on right port
          ...
          const SystemFlag7800 = $80 : rem Console detection constants
          const SystemFlagColorBWOverride = $40 : rem Bit 7: 1=7800 console, 0=2600 console
          const SystemFlagPauseButtonPrev = $20 : rem Bit 6: 1=Color/B&W override active (7800 only)
          rem Bit 5: 1=Pause button was pressed previous frame
          const SystemFlagGameStatePaused = $10
          const SystemFlagGameStateEnding = $08 : rem Bit 4: 1=Game paused (0=normal play)
          rem Bit 3: 1=Game ending (transition to winner screen)
          const ConsoleDetectD0 = $2C : rem Bits 0-2: Reserved for future use
          const ConsoleDetectD1 = $A9 : rem $D0 value for 7800 detection
          rem $D1 value for 7800 detection
          
          const ClearQuadtariDetected = $7F : rem Bit mask constants for clearing bits
          const ClearPlayers34Active = $BF : rem $FF - $80: Clear bit 7 (QuadtariDetected)
          const ClearSystemFlag7800 = $7F : rem $FF - $40: Clear bit 6 (Players34Active)
          const ClearSystemFlagColorBWOverride = $BF : rem $FF - $80: Clear bit 7 (SystemFlag7800)
          const ClearSystemFlagPauseButtonPrev = $DF : rem $FF - $40: Clear bit 6 (ColorBWOverride)
          const ClearSystemFlagGameStatePaused = $EF : rem $FF - $20: Clear bit 5 (PauseButtonPrev)
          const ClearSystemFlagGameStateEnding = $F7 : rem $FF - $10: Clear bit 4 (GameStatePaused)
          rem $FF - $08: Clear bit 3 (GameStateEnding)
          const SetQuadtariDetected = $80 : rem Bit mask constants for setting bits
          const SetSystemFlag7800 = $80 : rem Set bit 7 (QuadtariDetected)
          const SetSystemFlagColorBWOverride = $40 : rem Set bit 7 (SystemFlag7800)
          const SetSystemFlagPauseButtonPrev = $20 : rem Set bit 6 (ColorBWOverride)
          const SetSystemFlagGameStatePaused = $10 : rem Set bit 5 (PauseButtonPrev)
          ...
          const SetLeftPortGenesis = $01 : rem Bit mask constants for controller port detections
          ...
          rem
          const GravityPerFrame = 1 : rem Physics Tunables
          rem Pixels/frame added to vertical velocity when gravity
          const BounceDampenDivisor = 2 : rem   applies
          rem Divisor applied to velocity on bounce reflect (for minimal
          const CurlingFrictionCoefficient = 4 : rem   velocity reduction)
          ...
          const HitstunFrames = 10 : rem Pixels/frame impulse applied on hit knockback
          const MissileSpawnOffsetLeft = 4 : rem Frames of hitstun after a missile hit
          const MissileSpawnOffsetRight = 12 : rem Pixels to the left when facing left at spawn
          const MissileAABBSize = 4 : rem Pixels to the right when facing right at spawn
          const PlayerSpriteHalfWidth = 8 : rem Missile bounding box size (square)
          const PlayerSpriteHeight = 16 : rem Half-width of player sprite used in AABB checks
          const PlayerSpriteWidth = 16 : rem Player sprite height used in AABB checks
          const PlayerCollisionDistance = 16 : rem Full width of double-width sprite (16 pixels)
          const MissileDefaultHeight = 1 : rem Collision detection distance (sprite width in pixels)
          const MissileMaxHeight = 8 : rem Default missile height when AOE (0) is reported
          const MinimumVelocityThreshold = 1 : rem Cap for missile height (TIA limit)
          rem Minimum velocity threshold for missile deactivation (pixels/frame)
          const ScreenInsetX = 16 : rem Missiles with velocity below this threshold are deactivated
          const ScreenUsableWidth = 128 : rem Usable X inset from each side (playable area starts at 16)
          const ScreenBottom = 192 : rem Usable width inside insets
          const ScreenTopWrapThreshold = 200 : rem Bottom pixel threshold
          rem
          ...
          const MusicTitle = 26
          const MusicAtariToday = 27 : rem Title song Chaotica (index 26, loops)
          const MusicInterworldly = 28

          rem
          rem Sound Effect Constants
          const SoundAttackHit = 0 : rem Sound effect enumerated IDs (matching SoundPointers.bas indices 0-9)
          ...
          const MovementFrameRate = 60 : rem 60fps / 10fps = 6 frames
          const FramesPerSecond = 60 : rem 60fps movement updates
          rem 60fps frame rate
#endif

#ifdef TV_PAL  
          const AnimationFrameDelay = 5
          const MovementFrameRate = 50 : rem 50fps / 10fps = 5 frames
          const FramesPerSecond = 50 : rem 50fps movement updates
          ...
          const AnimationSequenceCount = 16 : rem Animation sequence structure constants
          const FramesPerSequence = 8 : rem 16 animation sequences (0-15)
          rem
          ...
          const SubpixelBits = 8 : rem   subpixel (0..255).
          const SubpixelScale = 256 : rem 8 bits of subpixel precision (0..255)
          rem
