...
;; Bank14MaxSongID (duplicate) = 6
          ;; Lowest song ID stored in Bank 1 music bank
;; Bank0MinSongID (duplicate) = Bank14MaxSongID + 1
...
;; PlayerSpriteHalfWidth (duplicate) = 8
          ;; Half-width of player sprite used in AABB checks
PlayerSpriteHeight = 16
          ;; Player sprite height used in AABB checks
;; PlayerSpriteWidth (duplicate) = 16
          ;; Player sprite width used in AABB checks
;; ScreenWidth (duplicate) = 160
          ;; Total hardware screen width in pixels
;; ScreenLeftMargin = (duplicate) 16
          ;; Left margin reserved for off-playfield area
;; ScreenRightMargin = (duplicate) 16
          ;; Right margin reserved for off-playfield area
;; PlayerLeftEdge = (duplicate) ScreenLeftMargin
          ;; Leftmost X position for a player sprite
;; PlayerRightEdge = (duplicate) ScreenWidth - ScreenRightMargin - PlayerSpriteWidth
          ;; Rightmost X position for a player sprite
;; PlayerWrapOvershoot = (duplicate) PlayerSpriteHalfWidth
          ;; Allow sprites to move partially off-screen before wrapping
;; PlayerLeftWrapThreshold = (duplicate) PlayerLeftEdge - PlayerWrapOvershoot
          ;; × value that triggers wrap-around from left edge
;; PlayerRightWrapThreshold = (duplicate) PlayerRightEdge + PlayerWrapOvershoot
          ;; × value that triggers wrap-around from right edge
          ;; NOTE: = definitions for these constants are in Preamble.s
...
.if TVStandard == 1
;; AnimationFrameDelay = 5 (duplicate)
;; MovementFrameRate = 50 (duplicate)
          ;; 50fps / 10fps = 5 frames
;; FramesPerSecond = 50 (duplicate)
          ;; 50fps movement updates
;; TitleParadeDelayFrames = 200 (duplicate)
          ;; Parade starts after ~4 seconds at 50fps
;; WinScreenAutoAdvanceFrames = 500 (duplicate)
          ;; Winner screen auto-advance after 10 seconds
;; GuardTimerMaxFrames = 50 (duplicate)
          ;; Guard duration capped at 1 second
;; HarpyFlapCooldownFrames = 33 (duplicate)
          ;; ~0.66 seconds between Harpy flaps (1.5 flaps/sec)
.fi
