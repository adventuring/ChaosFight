...
CheckAttackHit
  ...
  if PlayerX(defender_id) >= hitbox_left && PlayerX(defender_id) <= hitbox_right && 
     PlayerY(defender_id) >= hitbox_top && PlayerY(defender_id) <= hitbox_bottom then
    hit = 1
  else
    hit = 0
  endif
...
ProcessAttackerAttacks
  ...
  for defender = 0 to 3
    ...
    if hit then
      gosub ApplyDamage attacker_id, defender
    endif
...
rem Placeholder for damage indicator
ShowDamageIndicator
  rem Visual feedback for damage (to be implemented)
  return
...
UpdatePlayerGuard
  ...
  rem Decrement guard timer if active
  if (PlayerState(player_id) & %00000010) != 0 then
    guard_timer = guard_timer - 1
    if guard_timer <= 0 then
      rem End guard state
      PlayerState(player_id) = PlayerState(player_id) & %11111101
    endif
  endif
