...
MainLoopModePublisherPrelude
          rem CRITICAL: on gameMode gosub now pushes return address first, then JMPs to target
          rem So when MainLoopModePublisherPrelude starts, the stack has 2 bytes (return to ongosub0)
          rem gosub PublisherPreludeMain bank14 is a far call (pushes 4-byte encoded return)
          rem When PublisherPreludeMain returns with return otherbank, it pops 2 bytes from the encoded return address
          rem This leaves the 2-byte return address from on gameMode gosub on the stack
          rem So we must use return thisbank (RTS) to pop that 2-byte address
...
MainLoopModeGameMainContinue
          rem CRITICAL: on gameMode gosub is a near call (pushes 2-byte return address)
          rem Must use return thisbank to match the near call, not return thisbank
