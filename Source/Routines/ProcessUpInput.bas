...
ProcessUpInput
          ...
          rem
          rem Called Routines: BernieJump (bank13), HarpyJump (bank13), PlayfieldRead (bank16)
          rem
...
PUI_ProcessUp
          ...
          if playerCharacter[temp1] = 13 then PUI_RoboTitoAscend

          rem Check Bernie fall-through (Character 0)
          if playerCharacter[temp1] = 0 then PUI_BernieFallThrough

          rem Check Harpy flap (Character 6)
          if playerCharacter[temp1] = 6 then PUI_HarpyFlap
...
PUI_RoboTitoAscend
          ...
          if temp1 then PUI_RoboTitoLatch
          let temp1 = currentPlayer
          rem Clear latch if DOWN pressed (check appropriate port)
          if temp1 & 2 = 0 then PUI_CheckJoy0Down
