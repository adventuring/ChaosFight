# Link Search Results - AtariAge Forums & Steam

## ✅ Complete Forum Links (Already in Codebase)

These forum topic links are already complete and working:

| Game | Forum Topic URL | Topic ID |
|------|----------------|----------|
| Bernie and the Tower of Doom | `https://forums.atariage.com/topic/345295-bernie-the-tower-of-doom-done/` | 345295 |
| Dragon's Cache & Descent | `https://forums.atariage.com/topic/301656-dragons-cache-descent/` | 301656 |
| Dragon's Havoc | `https://forums.atariage.com/topic/331490-dragons-havoc-homebrew-side-scrolling-shooter/` | 331490 |
| Game of the Bear | `https://forums.atariage.com/topic/325262-game-of-the-bear/` | 325262 |
| Game of the Bear 2 | `https://forums.atariage.com/topic/331618-game-of-the-bear-2-much-to-bear/` | 331618 |
| Game of the Bear 3 | `https://forums.atariage.com/topic/345257-game-of-the-bear-polar-opposites/` | 345257 |

---

## ❌ Incomplete Links Requiring Search

### AtariAge Forum Topics (Missing Topic Numbers)

These links currently use `https://forums.atariage.com/topic/` without topic IDs:

#### High Priority (Referenced in Character Pages)

1. **RealSports Curling** (Ryan Witmer)
   - Current: `https://forums.atariage.com/topic/`
   - Location: `25/characters/curling-sweeper.html`, `25/manual/Characters.html`
   - Search: "RealSports Curling" Ryan Witmer Atari 5200

2. **Grizzards** (Zephyr & Bruce-Robert Pocock)
   - Current: `https://forums.atariage.com/topic/`
   - Location: `25/characters/fat-tony.html`, `25/characters/megax.html`, `25/characters/radish-goblin.html`, `25/manual/Characters.html`
   - Search: "Grizzards" Atari 2600 homebrew

3. **Harpy's Curse** (Todd "Revontulli" Furmanski)
   - Current: `https://forums.atariage.com/topic/`
   - Location: `25/characters/harpy.html`, `25/manual/Characters.html`
   - Search: "Harpy's Curse" Revontulli Atari 2600

4. **Knight Guy** (Vladimir "vhzc" Zúñiga)
   - Current: `https://forums.atariage.com/topic/`
   - Location: `25/characters/knight-guy.html`, `25/manual/Characters.html`
   - Search: "Knight Guy" "In Another Castle" vhzc

5. **Magical Fairy Force** (Ryan Witmer)
   - Current: `https://forums.atariage.com/topic/`
   - Location: `25/characters/frooty.html`, `25/characters/pork-chop.html`, `25/manual/Characters.html`
   - Search: "Magical Fairy Force" Ryan Witmer Atari 2600

6. **Fairy Drop** (Ryan Witmer)
   - Current: `https://forums.atariage.com/topic/`
   - Location: `25/characters/frooty.html`, `25/characters/pork-chop.html`, `25/manual/Characters.html`
   - Search: "Fairy Drop" Ryan Witmer Atari 2600

7. **Ninjish Guy** (Vladimir "vhzc" Zúñiga)
   - Current: `https://forums.atariage.com/topic/`
   - Location: `25/characters/ninjish-guy.html`, `25/manual/Characters.html`
   - Search: "Ninjish Guy" "Low-Res World" vhzc

8. **Rob 'N' Banks** (Ryan Witmer)
   - Current: `https://forums.atariage.com/topic/`
   - Location: `25/characters/pork-chop.html`, `25/manual/Characters.html`
   - Search: "Rob N Banks" Ryan Witmer Atari 2600

9. **Ratcatcher** (Ryan Witmer)
   - Current: `https://forums.atariage.com/topic/`
   - Location: `25/characters/pork-chop.html`, `25/manual/Characters.html`
   - Search: "Ratcatcher" Ryan Witmer Atari 2600

10. **Robo Tito** (Author TBD)
    - Current: `https://forums.atariage.com/topic/`
    - Location: `25/characters/robo-tito.html`, `25/manual/Characters.html`
    - Search: "Robo Tito" "Tito el robotito" Atari 2600

11. **Phantasia** (Zephyr & Bruce-Robert Pocock - in development)
    - Current: `https://forums.atariage.com/topic/`
    - Location: `25/characters/nefertem.html`, `25/characters/radish-goblin.html`, `25/manual/Characters.html`
    - Search: "Phantasia" Zephyr Pocock development thread

12. **Ducks Away** (Blake "SmittyB" Smith)
    - Current: `https://forums.atariage.com/topic/`
    - Location: `25/characters/shamone.html`, `25/manual/Characters.html`
    - Search: "Ducks Away" SmittyB Blake Smith Atari 2600

13. **Chaos Fight Forum Thread** (General support)
    - Current: `https://forums.atariage.com/topic/`
    - Location: `25/about/index.html`
    - Search: "Chaos Fight" release thread or support thread

#### Suggested Search Queries

Use these Google searches to find the forum topics:

```
site:forums.atariage.com "RealSports Curling" Ryan Witmer
site:forums.atariage.com "Grizzards" Atari 2600
site:forums.atariage.com "Harpy's Curse" Revontulli
site:forums.atariage.com "Knight Guy" "In Another Castle"
site:forums.atariage.com "Magical Fairy Force" Ryan Witmer
site:forums.atariage.com "Fairy Drop" Ryan Witmer
site:forums.atariage.com "Ninjish Guy" "Low-Res World"
site:forums.atariage.com "Rob N Banks" OR "Rob 'N' Banks" Ryan Witmer
site:forums.atariage.com "Ratcatcher" Ryan Witmer
site:forums.atariage.com "Robo Tito" OR "Tito el robotito"
site:forums.atariage.com "Phantasia" Zephyr development
site:forums.atariage.com "Ducks Away" SmittyB Blake Smith
site:forums.atariage.com "Chaos Fight"
```

---

### Steam App Links (Missing App IDs)

#### Missing Steam App ID

1. **Magical Fairy Force Championship Adventure**
   - Current: `https://store.steampowered.com/app/`
   - Location: 
     - `25/characters/frooty.html` (line 41)
     - `25/characters/pork-chop.html` (line 42)
     - `25/manual/Characters.html` (line 542, 612)
   - Search: "Magical Fairy Force Championship Adventure" Steam store

#### Suggested Search Query

```
site:store.steampowered.com "Magical Fairy Force Championship Adventure"
```

Or search SteamDB:
- Visit: https://steamdb.info/
- Search for: "Magical Fairy Force Championship Adventure"

---

## 📋 Format for Forum Topic URLs

AtariAge forum topics use this format:
```
https://forums.atariage.com/topic/[TOPIC_ID]-[slug-title]/
```

Example:
- Topic ID: 345295
- Slug: "bernie-the-tower-of-doom-done"
- Full URL: `https://forums.atariage.com/topic/345295-bernie-the-tower-of-doom-done/`

---

## 📋 Format for Steam App URLs

Steam app URLs use this format:
```
https://store.steampowered.com/app/[APP_ID]/[Game-Name]/
```

Example:
- App ID: 413150 (for Stardew Valley)
- Full URL: `https://store.steampowered.com/app/413150/Stardew_Valley/`

---

## 🔍 Next Steps

1. **Search for Missing Forum Topics:**
   - Use the suggested search queries above
   - Visit each forum thread
   - Copy the complete URL including topic ID

2. **Search for Steam App:**
   - Search Steam store directly
   - If not found, the game may not be on Steam
   - Consider removing link or noting "not available on Steam"

3. **Update HTML Files:**
   - Replace all instances of incomplete URLs
   - Verify links work after updating

4. **Test All Links:**
   - Visit each updated link to ensure it works
   - Check that topic IDs are correct

---

## 📝 Notes

- Some games may not have public forum threads if they're still in development
- Some games may not be available on Steam
- Consider adding a note like "Forum thread not yet available" for games without threads
- The Chaos Fight forum thread needs to be created if it doesn't exist yet

---

**Last Updated:** Link validation review  
**Status:** Incomplete - requires manual verification and updates



